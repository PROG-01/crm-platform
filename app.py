...
def edit_project(id):
    ...
    if request.method == 'POST':
        ...
        # Validate form data
        if not project_name or not client_id or status not in ['Not Started', 'In Progress', 'Completed']:
            return render_template('projects/edit.html', project=project[0], clients=clients, error='Please fill in all required fields')

        # Verify that the selected client belongs to the logged-in user
        client_row = db.execute("SELECT id FROM clients WHERE id = ? AND user_id = ?", client_id, session['user_id'])

        if not client_row:
            return render_template('projects/edit.html', project=project[0], clients=clients, error='Invalid client selection')
...
def delete_project(id):
    ...
    # Delete project for the logged-in user
    db.execute("DELETE FROM projects WHERE id = ? AND user_id = ?", id, session['user_id'])
